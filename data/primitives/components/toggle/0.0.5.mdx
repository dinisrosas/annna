...
<PropsTable
	data={[
		...
		},
		{
			...
			description: (
				<span>
					The controlled toggled state of the toggle button. Must be used in conjunction with in
					conjunction with <Code>onToggledChange</Code>.
				</span>
			)
		},
		{
			...
			typeSimple: 'function',
			description: 'Event handler called when the toggled state of the toggle button changes.'
		},
		{
			name: 'disabled',
			type: 'boolean',
			description: (
				<span>
					When <Code>true</Code>, prevents the user from interacting with the toggle button.
				</span>
			)
		}
	]}
