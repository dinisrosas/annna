...
export default function CaseStudy({ frontmatter, code }: CaseStudyPage) {
	...
	return (
		<>
			<TitleAndMetaTags
				title={`${frontmatter.metaTitle} – Casos de Estudo | Annna`}
			...
			<Container size={{ '@initial': 2, '@bp2': 3 }}>
				<Section>
					...
					>
						<Box>
							<MarketingCaption css={{ mb: '$1' }}>Caso de estudo</MarketingCaption>
...
export async function getStaticPaths() {
	...
	return {
		paths: frontmatters.map((frontmatter) => ({
			params: { slug: frontmatter.slug.replace('casos-de-estudo/', '') }
		})),
		fallback: false
	};
}
